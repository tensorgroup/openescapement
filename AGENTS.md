...
- **`esc init` detects existing instruction files** (`internal/cli/initscan.go`) and, on a TTY, may offer to write the placeholder marker at a chosen position (`internal/cli/initoffer.go`). It never writes rendered policy, only the marker, and never touches a file with uncommitted changes: git is the undo mechanism for anything init writes, so init does not write where git cannot undo it.
