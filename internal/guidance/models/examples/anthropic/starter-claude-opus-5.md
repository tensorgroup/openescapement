...
- Use `claude-opus-5` as the default frontier model for planning, plan-checking, and reviewing multi-file changes (1M-token context).
...
- Escalate to `claude-fable-5` only when Opus 5 genuinely stalls: Fable 5 costs materially more per token and runs slower.
